...
Exceptions are runtime errors that disrupt normal program flow. When Python encounters an error, it "raises" an exception.
...
# Good - minimal try, specific exceptions
try:
    file = open('data.txt')
except FileNotFoundError:
    print("File not found")
